...
module hs_fifo #(
    ...
    localparam integer SafePtrWidth = (Depth > 1) ? $clog2(Depth) : 1;

    typedef logic [flw_hs.W-1:0] data_t;
    typedef logic [SafePtrWidth-1:0] ptr_t;
    typedef logic [$clog2(Depth+1)-1:0] util_t;

    localparam ptr_t AddrMax = ptr_t'(Depth - 1);

    localparam integer EntryWidth = flw_hs.W + 1;
    ...
    (* syn_ramstyle = "no_rw_check" *) entry_t fifo[Depth];
